...
export function cleanPaper(raw) {
    if (!raw || typeof raw.title !== "string" || !raw.title.trim())
        throw Error("Every paper needs a title.");
    ...
    return {
        ...
        year:
            Number.isInteger(Number(raw.year)) && Number(raw.year) > 0
                ? Number(raw.year)
                : null,
...
export function deduplicate(records) {
    ...
    for (const raw of records) {
        const p = cleanPaper(raw),
            title = titleKey(p.title);
        ...
        if (existing) {
            ...
            if (!existing.doi && p.doi) {
                existing.doi = p.doi;
                existing.url = p.url;
                dois.set(p.doi, existing);
            }
        } else {
            result.push(p);
            if (p.doi) dois.set(p.doi, p);
            titles.set(title, p);
        }
    }
    return result;
...
export function search(
    papers,
    { query = "", topic = "", sort = "citations", saved = null } = {},
) {
    const terms = query.toLowerCase().trim().split(/\s+/).filter(Boolean);
    const rows = papers.filter(
        (p) =>
            (!topic || p.topics.includes(topic)) &&
            (!saved || saved.has(key(p))) &&
            terms.every((t) =>
                [p.title, p.doi, p.journal, ...p.authors]
                    .join(" ")
                    .toLowerCase()
                    .includes(t),
            ),
